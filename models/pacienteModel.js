...
class PacienteModel {
    static fetchAll(queryData, callback) {
        ...
        const search_query = search_value ? ` WHERE nome LIKE '%${search_value}%' OR cpf LIKE '%${search_value}%' OR data_nascimento LIKE '%${search_value}%' OR telefone LIKE '%${search_value}%' OR email LIKE '%${search_value}%' OR endereco LIKE '%${search_value}%'` : '';
    ...
    static submitData(data, callback) {
        ...
        if (action === 'Insert') {
            query = `INSERT INTO pacientes (nome, cpf, data_nascimento, telefone, email, endereco) VALUES (?, ?, ?, ?, ?, ?)`;
            queryData = [nome, cpf, data_nascimento, telefone, email, endereco];
